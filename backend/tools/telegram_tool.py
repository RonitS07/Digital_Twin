...
def delete_telegram_webhook():
    """Delete any registered webhook so getUpdates (long-polling) can work."""
    if not TELEGRAM_BOT_TOKEN:
        return
    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/deleteWebhook"
        r = requests.post(url, json={"drop_pending_updates": False}, timeout=10)
        if r.ok:
            logger.info("[Telegram] Webhook deleted — long-polling mode active.")
        else:
            logger.warning(f"[Telegram] Failed to delete webhook: {r.text}")
    except Exception as e:
        logger.warning(f"[Telegram] deleteWebhook error (non-fatal): {e}")
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 409:
                logger.warning(
                    ...
                    "Deleting webhook and backing off for 10s..."
                )
